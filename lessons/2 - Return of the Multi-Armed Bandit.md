...
&nbsp;&nbsp;&nbsp;Everyone and welcome back to this class reinforcement learning in python. In this lecture, we are going to summarize everything we learned in the section and draw a few important conclusions. In this section, we learned about the various approaches to the explore-exploit dilemma. We looked at the Epsilon greedy approach, the upper confidence bound approach, the optimistic initial values approach, and the Bayesian approach. We looked at the somewhat unusual scenario where the rewards were Gaussian distributed. These methods are usually used on binary outcomes but we already looked at that in my A/B testing course. So doing it again would not have been as interesting.

![](../Assets/photos/Return%20of%20the%20Multi-Armed%20Bandit_101.png)

&nbsp;&nbsp;&nbsp;One common question I get is why don't we use these methods on real data? In fact, this is a great quiz question. So if you haven't thought about what the answer to this question might be, please take a moment to think about it.

![](../Assets/photos/Return%20of%20the%20Multi-Armed%20Bandit_102.png)

&nbsp;&nbsp;&nbsp;All right. So hopefully you thought about the answer to why we don't use these methods on real data. In this class here is the answer. As you know, one of the major applications of the bandit problem is comparing items. This can be an online advertising on a website and so forth. Real data in this context would comprise of people. Yes, real people. In other words, users of a website or users that see an online advertising.

What is the problem with that? Think of it this way. Let's suppose I own a million dollar business with millions of users that see my website or my advertised man. Do you think it would be a good idea for me, the business owner, to let you, a student who is only beginning to learn this material, have access to my website? I think the answer should be obvious. A big fat no.

![](../Assets/photos/Return%20of%20the%20Multi-Armed%20Bandit_103.png)

&nbsp;&nbsp;&nbsp;As a business owner, the only way you would have access to write code for my website is if you are my employee and obviously that would require you to take a technical interview, pass the interview, demonstrate that you are capable at your job, and so forth. As a business owner, I would not allow you to have access to my customers because that would simply be impossible. In fact, we would also have to consider the fact that there are thousands of other students who need to do the same exercise.

![](../Assets/photos/Return%20of%20the%20Multi-Armed%20Bandit_104.png)

&nbsp;&nbsp;&nbsp;OK. You might think, well fine, I'll just make my own website. Sure you can do that. But then how do you get the thousands or millions of users that you need in order to test your algorithm? Do you think you can do this in a week or at least in a timeframe that is reasonable while you take this course again? The answer is a big fat no.

I assure you as a longtime web developer myself, starting a business and getting users is not easy. People fail at this very thing every single day.

![](../Assets/photos/Return%20of%20the%20Multi-Armed%20Bandit_105.png)

&nbsp;&nbsp;&nbsp;In fact, there is yet another problem. Even if you could start your own business and gain a million users in a week, what is the problem with using these algorithms on real users? Again, this is like a little mini quiz. So please take a moment to think about this by yourself and come back when you think you have the answer.

![](../Assets/photos/Return%20of%20the%20Multi-Armed%20Bandit_106.png)

&nbsp;&nbsp;&nbsp;All right. So hopefully you gave some thought as to why testing these algorithms on real users won't work. Remember that the goal of the section was to test the efficacy of these algorithms. In order to do that, we had to know the true answer. For example, if you have three slot machines with true average rewards 1, 2, and 3, you want to make sure your algorithm converges to the slot machine with reward 3.

But what about real online advertisements and real users? How would you test whether or not you arrived at the right answer? In order to do that, you would have to know the true click through rate of each advertisement. And obviously you don't know that. When a designer creates an advertisement, they can't tell you what the advertisement's true click through rate is. Therefore, even if you could start your own business in a week and gain a million users, which is not likely, it would all be for nothing because you don't even know the right answer.

![](../Assets/photos/Return%20of%20the%20Multi-Armed%20Bandit_107.png)

&nbsp;&nbsp;&nbsp;One thing I really like about the bandit algorithms is that they give you a totally different perspective on machine learning. If you think about a typical machine learning algorithm like k-means clustering or SVM, usually there are two modes of operation: training and prediction. In order to train the model, we call fit with X_train and y_train as our training data. Then at a later time, we can use the model to make predictions on future data by calling the predict function.

However, this paradigm doesn't fit the bandit algorithms. In fact, bandit algorithms are an example of online learning. With online learning algorithms, the model learns after it collects each data point. Also, each data point is separated in time. You don't have all the data points nicely organized in matrices called X_train and y_train. The data points simply appear randomly, like when a user sees your advertisement. As you saw, each model updates itself after collecting each data point, and any future action taken by the model takes into account all of the data that was previously collected.

This is very different from offline algorithms like k-means and SVM. With these, we usually cannot update the model after collecting a single data point as it would be very computationally expensive. Obviously, you have to know a little bit about how these algorithms work to understand why that is the case. If you don't, you can just take what I've said at face value or investigate it by yourself.

Later in this course, you'll see how we can turn a fully formed reinforcement learning agent into an online learner. Again, these are great because that means our model can adapt in real time and learn as it goes. There's no need to train models from scratch which can take a very long time especially in the case of deep learning models.

![](../Assets/photos/Return%20of%20the%20Multi-Armed%20Bandit_108.png)
...
&nbsp;&nbsp;&nbsp;In this lecture I want to discuss the merits of different bandit program designs. The reason I want to make this lecture is I've seen quite a few students get confused about this. The most important thing to remember is that everyone's design will be different. This is perfectly OK. There is no such thing as the correct design. Just like how there is no correct way to build something like say a mobile app framework, for example. Today there are two major mobile app frameworks, the iOS SDK and the Android SDK. Obviously they are very different. Is one of them correct and one of them incorrect? Clearly the answer should be no.

I see a lot of people also get confused about how to approach the exercises in this course even though I've mentioned that many times. So let me mention it again in the context of this lecture. Your goal isn't to copy my code or to make your code like my code. Your goal is to take the theory that is just the concepts and the mathematical equations and implement that in code by yourself without using any external coding resources. In other words, the code should come purely from your mind and nowhere else. When that happens your program is almost guaranteed to be different from everyone else's. And that is to be expected.

![](../Assets/photos/Return%20of%20the%20Multi-Armed%20Bandit_109.png)

&nbsp;&nbsp;&nbsp;OK so let's look at one approach that I see sometimes that has some problems. Often these come from people who write in the old style object-oriented way. My guess would be that they are influenced by languages such as Java. All right. So what does this design look like? Well it works off of our initial analogy that we are playing slot machines. So you might have a class called a slot machine which you will create multiple instances of, each with a different reward distribution. Then you might create a gambler object with just one instance and this guy plays the slot machines. In the gambler's code, he performs the algorithms that we learn in this section.

Now you might think this sounds perfect, right? In fact, this works exactly like our analogy. You are the gambler and there's one of you. All your calculations happen in your mind, so they belong to you. You go to the casino and you play multiple slot machines. Therefore, there should be multiple slot machine instances. So far so good.

![](../Assets/photos/Return%20of%20the%20Multi-Armed%20Bandit_110.png)

&nbsp;&nbsp;&nbsp;But wait, is slot machine really the right name? In fact, I don't think so. Remember that the idea of a slot machine was just used for intuition for introducing the idea of the multi-armed bandit problem. Think for a minute about another machine learning algorithm, neural networks. When I introduce a neuron that works, I might use an example such as a digit recognizer or a cat versus dog recognizer. Does that mean whenever you create a neural network you should name it digit recognizer or animal recognizer? I think you realize the answer is no. Should you call the input features image pixels? No. Should you call the output predictions animal type or digit score? No. So how should we name these things?

Well, let's look at one of the most popular APIs out there for building neural networks, Keras. Your model is simply an object of type Model. Your model is made up of layers. Each layer can be of a different type such as a dense layer, a convolutional layer, an LSTM layer, and so forth. The important point is to recognize that these are named after their mathematical concepts, not the examples I use to discuss the intuition. So please don't name your classes and objects based on the examples given in class.

![](../Assets/photos/Return%20of%20the%20Multi-Armed%20Bandit_111.png)

&nbsp;&nbsp;&nbsp;Second point, suppose we assume that it's OK to work with the names slot machine and gambler. Does it make sense to have multiple slot machines and one gambler? The slot machine is responsible for returning the reward and the gambler is responsible for doing the calculations. Again, you might think this makes perfect sense because it perfectly describes our example. However, remember that our example is only one example. It doesn't represent the real world environment in which we will test these algorithms.

In the real world, those slot machines might be advertisements. For each slot machine or advertisement, we will have model parameters representing their reward distribution. In other words, this is a many-to-many problem. For each advertisement, we will have a corresponding set of model parameters. So then why should each advertisement be represented by a single object but the so-called gambler is only one object which has to store and keep track of all the model parameters? While we can see from this that representing the problem as a many-to-one problem doesn't reflect the true structure of the problem.

![](../Assets/photos/Return%20of%20the%20Multi-Armed%20Bandit_112.png)

&nbsp;&nbsp;&nbsp;Now you might think, OK well this is reinforcement learning. Therefore there is an environment and there is an agent. So let's create one environment which implements multiple slot machines and one agent which performs the calculations we discussed previously. So now instead of a many-to-one you have a one-to-one. This is an OK approach but I still don't think it's ideal. The first reason is that it depends on concepts which we haven't discussed yet in this course. Usually when you're talking about bandit algorithms you do not think of them in terms of a general reinforcement learning problem. So that approach would be unusual at the very least.

Another problem with this approach is that it seems like a bad abstraction. The bandit problem is really a many-to-many because for each slot machine we have a model for that slot machine's reward distribution. Therefore having one environment and one agent makes it seem like a one-to-one problem. But in reality it's a many-to-many problem. So the interface is a little misleading and inaccurate. In addition, you have to keep track of which model goes with which advertisement on both and separately the environment and the agent, which will probably turn out to be inconvenient.

![](../Assets/photos/Return%20of%20the%20Multi-Armed%20Bandit_113.png)

&nbsp;&nbsp;&nbsp;OK so now you are at a point where we realize the following things: first, it may not be the best idea to name your objects after some introductory examples that were used in the classroom. Second, it may not be the best idea to disguise your problem as a many-to-one or one-to-one if in fact it's many-to-many.

OK so let's name our class Bandit after the mathematical concept. The question now is should we have separate classes for the so-called bandit and the so-called mathematical model? Again this is OK but it might be inconvenient to have to keep track of which bandit belongs to which model and to have one object which is the bandit or the slot machine with just one function which is the pull function. In other words, it might be more convenient to just have both of them in the same class as we did in this course.

![](../Assets/photos/Return%20of%20the%20Multi-Armed%20Bandit_114.png)

&nbsp;&nbsp;&nbsp;There is another major reason for this. But again we have to consider how we might actually use these algorithms in the real world. In the real world, your bandit problem is no longer a simulation. Remember that the reason we are doing a simulation in this course is because there is no way you're going to start a business with millions of users before you can get through this course.

So let's differentiate between these two different scenarios. In this course we are doing a simulation but in the real world we are working as an employee of a large business that has real users clicking on real stuff. Usually your application will sit on a web server. So you'll be using a web server framework. Some popular models for this are the Model View Controller or the Actor framework.

Now this is not a backend engineering course, so I don't expect you to know what these mean but suffice it to say what you'll have is for each request an object will be instantiated to handle that request. That object will do two things, the two things we talk about in this course.

First, it will accept some user action from the request. This would be the equivalent of receiving a reward after pulling a slot machine. Once we've obtained that user action or reward, we will use it to update our model. That might be some values in a database.

In other words, the bandit that we designed in this course mirrors that scenario. Each bandit object takes care of two things. First, it handles a reward from a so-called slot machine. And second, it updates the model parameters corresponding to that so-called slot machine.

The major difference is that in our case it's unnecessary and wasteful to instantiate a new object for each pull or to use a database to store the results.

![](../Assets/photos/Return%20of%20the%20Multi-Armed%20Bandit_115.png)

&nbsp;&nbsp;&nbsp;In the end, it doesn't really matter what you design because nobody should ever see your design unless it's for your job. In that scenario, it doesn't really matter what I say; it matters what your boss or your manager says. Therefore the ultimate metrics for your success are things like does it pass a code review by your peers? Does it perform well in terms of your project's requirements such as speed or responsiveness and so forth? Does it perform correctly? Is it easily maintainable by other programmers on your team?

And remember this is still context specific because the requirements that matter for your team will not be the same as the requirements that matter for some other team at some other company.

Ultimately, it's not a good idea to try and forcefully convince others that your design is correct. Instead, let the metrics do the talking.

![](../Assets/photos/Return%20of%20the%20Multi-Armed%20Bandit_116.png)
